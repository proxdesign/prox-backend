#!/usr/bin/env python3
"""
Script to add furniture products using Rainforest API
"""
import requests
import json
import os
...
RAINFOREST_API_KEY = os.getenv('RAINFOREST_API_KEY', '1BB720F0F7B8459B89A3DCCBC83AD0C0')
...
def fetch_products_for_category(category: str, max_products: int = 10) -> List[Dict]:
        ...
        if 'search_results' in data:
            for item in data['search_results']:
                product = {
                    ...
                    'reviews_count': item.get('reviews_total', 0),
                    'asin': item.get('asin', ''),
                    'category': category,
                    'url': item.get('link', '')
                }
                
                if product['title'] and product['image_url']:
...
def extract_price(price_data) -> float:
    """Extract numeric price from price data"""
    if isinstance(price_data, (int, float)):
        return float(price_data)
    
    if isinstance(price_data, dict):
        price_str = str(price_data.get('value', '0'))
    else:
...
def main():
    """Main function to fetch and save all furniture products"""
    all_products = []
    ...
    for category in FURNITURE_CATEGORIES:
        ...
        products = fetch_products_for_category(category, max_products=15)
    ...
    if all_products:
        ...
        print("✅ Successfully added furniture products!")
